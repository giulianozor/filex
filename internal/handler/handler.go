...
func New(globalFS *fslib.FS, cfg *config.Config, staticFS http.FileSystem, authStore *authlib.Store, users map[string]*userEntry, configPath string, version string) *Handler {
	...
	runtimeFavs[""] = normalizeFavourites(globalFavs)
	for _, u := range cfg.Users {
		...
		runtimeFavs[u.Username] = normalizeFavourites(uf)
	}
...
func (h *Handler) registerRoutes(staticFS http.FileSystem) {
	...
	h.mux.HandleFunc("/api/favourites/update", h.authMiddleware(h.handleFavouritesUpdate))
...
func (h *Handler) persistFavourites(key string, favs []config.Favourite) error {
	...
	return h.cfg.Save(h.configPath)
}
...
func (h *Handler) handleFavouritesAdd(w http.ResponseWriter, r *http.Request) {
	...
	req.Path = normalizeVirtualPath(req.Path)
	...
	h.runtimeFavs[key] = normalizeFavourites(append(favs, config.Favourite{Name: req.Name, Path: req.Path}))
	updated := h.copyFavs(key)
	h.favMu.Unlock()
	if err := h.persistFavourites(key, updated); err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
	writeJSON(w, map[string]string{"status": "ok"})
...
func (h *Handler) handleFavouritesRemove(w http.ResponseWriter, r *http.Request) {
	...
	h.runtimeFavs[key] = normalizeFavourites(updated)
	saveFavs := h.copyFavs(key)
	h.favMu.Unlock()
	if err := h.persistFavourites(key, saveFavs); err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
	writeJSON(w, map[string]string{"status": "ok"})
...
func (h *Handler) handleFavouritesUpdate(w http.ResponseWriter, r *http.Request) {
	...
	key := h.favKeyForRequest(r)
	h.favMu.Lock()
	h.runtimeFavs[key] = normalizeFavourites(req.Favourites)
	updated := h.copyFavs(key)
	h.favMu.Unlock()
	if err := h.persistFavourites(key, updated); err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
